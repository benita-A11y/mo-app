...
(async () => {
  ...
  ws.onmessage = (e) => {
    ...
    if (m.method === 'Runtime.consoleAPICalled') {
      const args = (m.params.args || []).map((a) => a.value || a.description || '').join(' ').slice(0, 400);
      consoleMsgs.push(m.params.type + ': ' + args);
    }
    if (m.method === 'Runtime.exceptionThrown') {
      const ex = m.params.exceptionDetails && m.params.exceptionDetails.exception;
      consoleMsgs.push('EXC: ' + ((ex && ex.description) || '').slice(0, 400));
    }
  ...
  const evalJS = async (expression) => {
    ...
    if (r.exceptionDetails) throw new Error('EXC-EVAL: ' + JSON.stringify(r.exceptionDetails).slice(0, 400));
  ...
  ws.onopen = async () => {
    try {
      ...
      await sleep(200);
      const body = await evalJS(`document.body.innerHTML.slice(0, 3000)`);
      console.log('BODY_START=' + body);
