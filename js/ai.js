...
严格输出 JSON：{"action":"today|backlog|drop","slot":"槽位key或null","reason":"≤18字"}`;
  ...
  const LIB = {
    ...
    today_empty: [
      () => '今天没有安排，可以休息或者去待办看看。',
      () => '今天空空的，正好给自己留点空白。想做点什么的话，待办里随时有。',
      () => '没有任务的一天。去晒晒太阳，或者就发会儿呆，都很好。'
    ],
    mood_low: [
      () => '今天状态偏低。我只帮你安排1件最重要的事。做完就休息。',
      () => '感觉今天有点累。那我们就只做一件，剩下的交给明天。',
      () => '状态不高的时候，少即是多。一件就好，做完就放过自己。'
    ],
    ...
    /* —— 本月重点 / AI 拆解 —— */
    month_highlight_set: [ c => `已设为本月重点：${c.task}。需要我帮你把它拆成几周的小步子吗？` ],
    month_auto_done: [ c => `已为「${c.task}」生成 ${c.weeks} 周计划与每日任务，已自动放进本周和今天的清单里啦 💜` ],
    month_no_highlight: [ () => '先把一个目标拖到「本月重点」，再点 ✦ AI 拆解，我就能帮你规划整月啦。' ],
